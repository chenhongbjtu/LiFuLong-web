    ...
    <div style="max-width: 90vw;" >
      ...
      <div class="justify-center" style="margin-left:10px;">
        <q-card style="max-width:400px;background-color:#77C0C7" v-for="table in tables" v-bind:key="table.id" inline class="q-ma-sm">
          <q-card-title>
            <a href="javascript:" @click.prevent="goPage('/order-detail-by-table/'+table.id)" >地址:{{table.table_number}}</a>
          ...
          <q-list>
            <q-item v-for="l in table.inner_items" v-bind:key="l.item_id">
              ...
              <q-item-side>
                  <q-chip v-if="l.status==='readytoserve'" small color="deep-orange-6">完成待揽收</q-chip>
                  <q-chip v-else-if="l.status==='unassigned'" small color="brown-4">未分配</q-chip>
                  <q-chip v-else-if="l.status==='assigned'" small color="lime-14">备货中</q-chip>
                  <q-chip v-else-if="l.status==='served'" small color="light-green-10">已送达</q-chip>
                  <q-chip v-else small color="brown">{{l.status}}</q-chip>
              </q-item-side>
              <q-item-side>
                <q-btn @click="served(l.item_id)" v-if="l.status==='readytoserve'" icon="speaker_notes_off" dense glossy color="purple-13" label="已送达" />
              </q-item-side>
           </q-item>
          </q-list>
        </q-card>
