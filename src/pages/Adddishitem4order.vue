<template>
  <q-page class="flex flex-center">
    <div style="width: 500px; max-width: 90vw; border:1px dashed #000;" >
      ...
      <q-btn :disabled="order_status==='finished'"  title="加产品" label="加产品"  color="primary" @click="addItem" style="margin-top:15px;margin-left:10px;margin-bottom: 10px;" icon="add" />
      <q-select
      filter
    ...
    <!-- <q-input style="width:30px;margin-left: 10px;" v-model="count" type="number" float-label="数量" /> -->
       ...
       <q-input style="margin-left: 15px;" v-model="description" float-label="运输方式及运输时间（如：常温运输-3小时）" />
    </div>
  </q-page>
</template>
